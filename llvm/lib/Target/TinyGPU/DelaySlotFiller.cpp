...
static cl::opt<bool> DisableDelaySlotFiller(
  "disable-tinygpu-delay-filler",
  cl::init(false),
  cl::desc("Disable the TinyGPU delay slot filler."),
  cl::Hidden);

namespace {
  struct Filler : public MachineFunctionPass {
    const TinyGPUSubtarget *Subtarget = nullptr;

    static char ID;
    Filler() : MachineFunctionPass(ID) {}

    StringRef getPassName() const override { return "TinyGPU Delay Slot Filler"; }

    bool runOnMachineBasicBlock(MachineBasicBlock &MBB);
    bool runOnMachineFunction(MachineFunction &F) override {
      bool Changed = false;
      Subtarget = &F.getSubtarget<TinyGPUSubtarget>();
...
/// createTinyGPUDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in TinyGPU MachineFunctions
///
FunctionPass *llvm::createTinyGPUDelaySlotFillerPass() {
  return new Filler;
}
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  Subtarget = &MBB.getParent()->getSubtarget<TinyGPUSubtarget>();
