...
static cl::opt<bool>
DisableLeafProc("disable-TinyGPU-leaf-proc",
                cl::init(false),
                cl::desc("Disable TinyGPU leaf procedure optimization."),
                cl::Hidden);

TinyGPUFrameLowering::TinyGPUFrameLowering(const TinyGPUSubtarget &ST)
    : TargetFrameLowering(TargetFrameLowering::StackGrowsDown,
                          ST.is64Bit() ? Align(16) : Align(8), 0,
                          ST.is64Bit() ? Align(16) : Align(8),
                          /*StackRealignable=*/false) {}

void TinyGPUFrameLowering::emitSPAdjustment(MachineFunction &MF,
  ...
  DebugLoc dl;
  const TinyGPUInstrInfo &TII =
      *static_cast<const TinyGPUInstrInfo *>(MF.getSubtarget().getInstrInfo());
...
void TinyGPUFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  const TinyGPUInstrInfo &TII =
      *static_cast<const TinyGPUInstrInfo *>(Subtarget.getInstrInfo());
  const TinyGPURegisterInfo &RegInfo =
      *static_cast<const TinyGPURegisterInfo *>(Subtarget.getRegisterInfo());
  ...
  // The TinyGPU ABI is a bit odd in that it requires a reserved 92-byte
  ...
  // Adds the TinyGPU subtarget-specific spill area to the stack
...
MachineBasicBlock::iterator TinyGPUFrameLowering::
...
StackOffset
TinyGPUFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
                                           Register &FrameReg) const {
  const TinyGPUSubtarget &Subtarget = MF.getSubtarget<TinyGPUSubtarget>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TinyGPURegisterInfo *RegInfo = Subtarget.getRegisterInfo();
  const TinyGPUMachineFunctionInfo *FuncInfo = MF.getInfo<TinyGPUMachineFunctionInfo>();
  ...
  // TinyGPU uses FP-based references in general, even when "hasFP" is
...
bool TinyGPUFrameLowering::isLeafProc(MachineFunction &MF) const
{
...
void TinyGPUFrameLowering::remapRegsForLeafProc(MachineFunction &MF) const {
  MachineRegisterInfo &MRI = MF.getRegInfo();
...
void TinyGPUFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  if (!DisableLeafProc && isLeafProc(MF)) {
    TinyGPUMachineFunctionInfo *MFI = MF.getInfo<TinyGPUMachineFunctionInfo>();
