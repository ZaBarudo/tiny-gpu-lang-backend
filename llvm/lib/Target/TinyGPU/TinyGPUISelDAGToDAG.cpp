...
//===--------------------------------------------------------------------===//
/// TinyGPUDAGToDAGISel - TinyGPU specific code to select TinyGPU machine
/// instructions for SelectionDAG operations.
///
namespace {
class TinyGPUDAGToDAGISel : public SelectionDAGISel {
  /// Subtarget - Keep a pointer to the TinyGPU Subtarget around so that we can
  /// make the right decision when generating code for different targets.
  const TinyGPUSubtarget *Subtarget = nullptr;

public:
  TinyGPUDAGToDAGISel() = delete;

  explicit TinyGPUDAGToDAGISel(TinyGPUTargetMachine &tm) : SelectionDAGISel(tm) {}
...
public:
  static char ID;
  explicit TinyGPUDAGToDAGISelLegacy(TinyGPUTargetMachine &tm)
      : SelectionDAGISelLegacy(ID, std::make_unique<TinyGPUDAGToDAGISel>(tm)) {}
};
...
SDNode* TinyGPUDAGToDAGISel::getGlobalBaseReg() {
...
bool TinyGPUDAGToDAGISel::SelectADDRri(SDValue Addr,
...
bool TinyGPUDAGToDAGISel::SelectADDRrr(SDValue Addr, SDValue &R1, SDValue &R2) {
...
bool TinyGPUDAGToDAGISel::tryInlineAsm(SDNode *N){
...
void TinyGPUDAGToDAGISel::Select(SDNode *N) {
  SDLoc dl(N);
...
/// SelectInlineAsmMemoryOperand - Implement addressing mode selection for
/// inline asm expressions.
bool TinyGPUDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
/// createTinyGPUISelDag - This pass converts a legalized DAG into a
/// TinyGPU-specific DAG, ready for instruction scheduling.
///
FunctionPass *llvm::createTinyGPUISelDag(TinyGPUTargetMachine &TM) {
  return new TinyGPUDAGToDAGISelLegacy(TM);
}
