//===- TinyGPUDisassembler.cpp - Disassembler for TinyGPU -----------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is part of the TinyGPU Disassembler.
//
//===----------------------------------------------------------------------===//
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeTinyGPUDisassembler() {
  // Register the disassembler.
  TargetRegistry::RegisterMCDisassembler(getTheTinyGPUTarget(),
                                         createTinyGPUDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheTinyGPUV9Target(),
                                         createTinyGPUDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheTinyGPUelTarget(),
                                         createTinyGPUDisassembler);
}
...
// depending on TinyGPURegisterInfo::getPointerRegClass.
...
DecodeStatus TinyGPUDisassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  if (STI.hasFeature(TinyGPU::FeatureV9))
  {
    Result = decodeInstruction(DecoderTableTinyGPUV932, Instr, Insn, Address, this, STI);
  }
  else
  {
    Result = decodeInstruction(DecoderTableTinyGPUV832, Instr, Insn, Address, this, STI);
  }
  if (Result != MCDisassembler::Fail) {
    Size = 4;
    return Result;
  ...
  Result =
      decodeInstruction(DecoderTableTinyGPU32, Instr, Insn, Address, this, STI);
