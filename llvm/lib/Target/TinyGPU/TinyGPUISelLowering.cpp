...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that TinyGPU uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
static bool CC_TinyGPU_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_TinyGPU_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
static bool CC_TinyGPU_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
...
static bool Analyze_CC_TinyGPU64_Full(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool Analyze_CC_TinyGPU64_Half(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool CC_TinyGPU64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_TinyGPU64_Full(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool CC_TinyGPU64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_TinyGPU64_Half(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_TinyGPU64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_TinyGPU64_Full(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_TinyGPU64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_TinyGPU64_Half(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

#include "TinyGPUGenCallingConv.inc"

// The calling conventions in TinyGPUCallingConv.td are described in terms of the
...
bool TinyGPUTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, Subtarget->is64Bit() ? RetCC_TinyGPU64
                                                       : RetCC_TinyGPU32);
}
...
SDValue
TinyGPUTargetLowering::LowerReturn_32(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    const SDLoc &DL, SelectionDAG &DAG) const {
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_TinyGPU32);
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    TinyGPUMachineFunctionInfo *SFI = MF.getInfo<TinyGPUMachineFunctionInfo>();
...
SDValue
TinyGPUTargetLowering::LowerReturn_64(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    const SDLoc &DL, SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_TinyGPU64);
...
SDValue TinyGPUTargetLowering::LowerFormalArguments_32(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineRegisterInfo &RegInfo = MF.getRegInfo();
  TinyGPUMachineFunctionInfo *FuncInfo = MF.getInfo<TinyGPUMachineFunctionInfo>();

  // Assign locations to all of the incoming arguments.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_TinyGPU32);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (Ins[InIdx].Flags.isSRet()) {
      if (InIdx != 0)
        report_fatal_error("TinyGPU only supports sret on the first parameter");
      // Get SRet from [%fp+64].
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      report_fatal_error("TinyGPUv8 does not handle f128 in calls; "
                         "pass indirectly");
    } else {
...
SDValue TinyGPUTargetLowering::LowerFormalArguments_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();

  // Analyze arguments according to CC_TinyGPU64.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_TinyGPU64);
  ...
  for (const CCValAssign &VA : ArgLocs) {
    ...
    assert(VA.isMemLoc());
    // The CC_TinyGPU64_Full/Half functions compute stack offsets relative to the
    // beginning of the arguments area at %fp+BIAS+128.
    unsigned Offset = VA.getLocMemOffset() + ArgArea;
    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
    // Adjust offset for extended arguments, TinyGPU is big-endian.
...
static bool isAnyArgRegReserved(const TinyGPURegisterInfo *TRI,
...
static void emitReservedArgRegCallError(const MachineFunction &MF) {
  ...
  F.getContext().diagnose(DiagnosticInfoUnsupported{
      F, ("TinyGPU doesn't support"
          " function calls if any of the argument registers is reserved.")});
}

SDValue
TinyGPUTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
...
bool TinyGPUTargetLowering::IsEligibleForTailCallOptimization(
...
SDValue
TinyGPUTargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_TinyGPU32);
  ...
  unsigned TF = isPositionIndependent() ? TinyGPUMCExpr::VK_TinyGPU_WPLT30
                                        : TinyGPUMCExpr::VK_TinyGPU_WDISP30;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee))
  ...
  const TinyGPURegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_TinyGPU32);
...
Register TinyGPUTargetLowering::getRegisterByName(const char* RegName, LLT VT,
...
//
// The TinyGPU v9 ABI requires that floating point arguments are treated the same
...
SDValue
TinyGPUTargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_TinyGPU64);

  CLI.IsTailCall = CLI.IsTailCall && IsEligibleForTailCallOptimization(
                                         CCInfo, CLI, DAG.getMachineFunction());

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_TinyGPU64 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::BCvt:
      ...
      // TinyGPU does not support i128 natively. Lower it into two i64, see below.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_TinyGPU64);
...
TargetLowering::AtomicExpansionKind TinyGPUTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
/// intCondCCodeToRcond - Convert a DAG integer condition code to a TinyGPU
/// rcond condition.
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a TinyGPU ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a TinyGPU
/// FCC condition.
...
TinyGPUTargetLowering::TinyGPUTargetLowering(const TargetMachine &TM,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On 32bit TinyGPU, we define a double-register 32bit register
  ...
  // TinyGPU doesn't have i1 sign extending load
  ...
  // TinyGPU doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // TinyGPU has no REM or DIVREM operations.
  ...
  // TinyGPU has no select or setcc: expand to SELECT_CC.
  ...
  // TinyGPU doesn't have BRCOND either, it has BR_CC.
  ...
  // ATOMICs.
  // Atomics are supported on TinyGPUV9. 32-bit atomics are also
  // supported by some Leon TinyGPUV8 variants. Otherwise, atomics
  // are unsupported.
  if (Subtarget->isV9()) {
    // TODO: we _ought_ to be able to support 64-bit atomics on 32-bit TinyGPUv9,
  ...
  if (!Subtarget->isV9()) {
    // TinyGPUV8 does not have FNEGD and FABSD.
...
bool TinyGPUTargetLowering::useSoftFloat() const {
...
EVT TinyGPUTargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                            EVT VT) const {
  if (!VT.isVector())
    return MVT::i32;
...
/// combiner.
void TinyGPUTargetLowering::computeKnownBitsForTargetNode
                                (const SDValue Op,
                                 KnownBits &Known,
...
SDValue TinyGPUTargetLowering::withTargetFlags(SDValue Op, unsigned TF,
...
SDValue TinyGPUTargetLowering::makeHiLoPair(SDValue Op,
...
SDValue TinyGPUTargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = getPointerTy(DAG.getDataLayout());

  // Handle PIC mode first. TinyGPU needs a got load for every variable!
  if (isPositionIndependent()) {
    ...
    if (picLevel == PICLevel::SmallPIC) {
      ...
      Idx = DAG.getNode(SPISD::Lo, DL, Op.getValueType(),
                        withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_GOT13, DAG));
    } else {
      // This is the pic32 code model, the GOT is known to be smaller than 4GB.
      Idx = makeHiLoPair(Op, TinyGPUMCExpr::VK_TinyGPU_GOT22,
                         TinyGPUMCExpr::VK_TinyGPU_GOT10, DAG);
    }
  ...
  case CodeModel::Small:
    // abs32.
    return makeHiLoPair(Op, TinyGPUMCExpr::VK_TinyGPU_HI,
                        TinyGPUMCExpr::VK_TinyGPU_LO, DAG);
  case CodeModel::Medium: {
    // abs44.
    SDValue H44 = makeHiLoPair(Op, TinyGPUMCExpr::VK_TinyGPU_H44,
                               TinyGPUMCExpr::VK_TinyGPU_M44, DAG);
    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
    SDValue L44 = withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_L44, DAG);
    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
  }
  case CodeModel::Large: {
    // abs64.
    SDValue Hi = makeHiLoPair(Op, TinyGPUMCExpr::VK_TinyGPU_HH,
                              TinyGPUMCExpr::VK_TinyGPU_HM, DAG);
    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
    SDValue Lo = makeHiLoPair(Op, TinyGPUMCExpr::VK_TinyGPU_HI,
                              TinyGPUMCExpr::VK_TinyGPU_LO, DAG);
    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
  }
  }
...
SDValue TinyGPUTargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? TinyGPUMCExpr::VK_TinyGPU_TLS_GD_HI22
                     : TinyGPUMCExpr::VK_TinyGPU_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? TinyGPUMCExpr::VK_TinyGPU_TLS_GD_LO10
                     : TinyGPUMCExpr::VK_TinyGPU_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? TinyGPUMCExpr::VK_TinyGPU_TLS_GD_ADD
                      : TinyGPUMCExpr::VK_TinyGPU_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? TinyGPUMCExpr::VK_TinyGPU_TLS_GD_CALL
                       : TinyGPUMCExpr::VK_TinyGPU_TLS_LDM_CALL);
    ...
    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? TinyGPUMCExpr::VK_TinyGPU_TLS_IE_LDX
                         : TinyGPUMCExpr::VK_TinyGPU_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               TinyGPUMCExpr::VK_TinyGPU_TLS_IE_HI22,
                               TinyGPUMCExpr::VK_TinyGPU_TLS_IE_LO10, DAG);
    ...
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT,
                       ...
                       withTargetFlags(Op,
                                       TinyGPUMCExpr::VK_TinyGPU_TLS_IE_ADD, DAG));
  }

  assert(model == TLSModel::LocalExec);
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, TinyGPUMCExpr::VK_TinyGPU_TLS_LE_LOX10, DAG));
...
SDValue TinyGPUTargetLowering::LowerF128_LibCallArg(SDValue Chain,
...
SDValue
TinyGPUTargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue TinyGPUTargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
...
LowerF128_FPEXTEND(SDValue Op, SelectionDAG &DAG,
                   const TinyGPUTargetLowering &TLI) {
...
static SDValue LowerBR_CC(SDValue Op, SelectionDAG &DAG,
                          const TinyGPUTargetLowering &TLI, bool hasHardQuad,
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On TinyGPU32, the size of the spill area is 92. Unfortunately,
...
SDValue TinyGPUTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
SDValue TinyGPUTargetLowering::bitcastConstantFPToInt(ConstantFPSDNode *C,
                                                    const SDLoc &DL,
                                                    SelectionDAG &DAG) const {
...
SDValue TinyGPUTargetLowering::PerformBITCASTCombine(SDNode *N,
...
SDValue TinyGPUTargetLowering::PerformDAGCombine(SDNode *N,
                                               DAGCombinerInfo &DCI) const {
  switch (N->getOpcode()) {
  default:
    break;
...
MachineBasicBlock *
TinyGPUTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
...
MachineBasicBlock *
TinyGPUTargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB,
...
//===----------------------------------------------------------------------===//
//                         TinyGPU Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TinyGPUTargetLowering::ConstraintType
TinyGPUTargetLowering::getConstraintType(StringRef Constraint) const {
...
TargetLowering::ConstraintWeight TinyGPUTargetLowering::
...
void TinyGPUTargetLowering::LowerAsmOperandForConstraint(
...
TinyGPUTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
...
bool
TinyGPUTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The TinyGPU target isn't yet aware of offsets.
  return false;
}
...
bool TinyGPUTargetLowering::useLoadStackGuardNode(const Module &M) const {
...
void TinyGPUTargetLowering::insertSSPDeclarations(Module &M) const {
  if (!Subtarget->isTargetLinux())
    return TargetLowering::insertSSPDeclarations(M);
}

void TinyGPUTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                        SDNode *Node) const {
