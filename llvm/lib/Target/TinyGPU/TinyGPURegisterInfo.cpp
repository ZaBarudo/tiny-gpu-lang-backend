...
static cl::opt<bool>
ReserveAppRegisters("TinyGPU-reserve-app-registers", cl::Hidden, cl::init(false),
                    cl::desc("Reserve application registers (%g2-%g4)"));

TinyGPURegisterInfo::TinyGPURegisterInfo() : TinyGPUGenRegisterInfo(SP::O7) {}
...
const uint32_t*
TinyGPURegisterInfo::getRTCallPreservedMask(CallingConv::ID CC) const {
...
BitVector TinyGPURegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  for (TargetRegisterClass::iterator i = SP::IntRegsRegClass.begin();
    ...
    if (MF.getSubtarget<TinyGPUSubtarget>().isRegisterReserved(*i))
...
bool TinyGPURegisterInfo::isReservedReg(const MachineFunction &MF,
                                      MCRegister Reg) const {
  return getReservedRegs(MF)[Reg];
}

const TargetRegisterClass*
TinyGPURegisterInfo::getPointerRegClass(const MachineFunction &MF,
                                      unsigned Kind) const {
  const TinyGPUSubtarget &Subtarget = MF.getSubtarget<TinyGPUSubtarget>();
...
bool
TinyGPURegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  const TinyGPUSubtarget &Subtarget = MF.getSubtarget<TinyGPUSubtarget>();
  const TinyGPUFrameLowering *TFI = getFrameLowering(MF);
